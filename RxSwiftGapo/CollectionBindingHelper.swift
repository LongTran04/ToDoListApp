...
open class CollectionBindingHelper<VM: IListViewModel>: NSObject, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    open func bindViewAndViewModel() {
        ...
        collectionView.rx.itemDeselected.asObservable()
            .subscribe(onNext: { [weak self] indexPath in
                self?.onItemDeselected(indexPath)
            }) => disposeBag
    ...
    private func onItemDeselected(_ indexPath: IndexPath) {
        guard let viewModel = viewModel else { return }
        let cellViewModel = viewModel.itemsSource[indexPath.row, indexPath.section]
        deselectedItemDidChange(cellViewModel)
    }
    ...
    open func deselectedItemDidChange(_ cellViewModel: CVM) { }
